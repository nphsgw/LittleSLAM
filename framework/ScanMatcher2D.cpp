...
bool ScanMatcher2D::matchScan(Scan2D &curScan) {
  ...
  /*
  1. 現在の姿勢(curScan.pose)と一つ前の姿勢(prevScan.pose)からこの2つの姿勢間での移動量を算出。
  2. 1.で求めた移動量を地図上に記録した最新の位置に加えて、予測位置を算出する。
      開始直後の場合、地図上の最新位置は0,0になる。
  3. 2.で求めた予測位置を基準に、ICPによる位置推定を行う。
  4. 3.の結果が閾値より小さければ成功
  */
...
void ScanMatcher2D::growMap(const Scan2D &scan, const Pose2D &pose) {
  ...
  // 点群地図pcmapに登録
  pcmap->addPose(pose);      // 新しい姿勢を追加
  pcmap->addPoints(scanG);   // 新しい点群を追加
  pcmap->setLastPose(pose);  // 最新姿勢を保存
