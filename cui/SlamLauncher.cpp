...
void SlamLauncher::run() {
  ...
  while (!eof) {
    if (odometryOnly) {  // オドメトリによる地図構築（SLAMより優先）
      if (cnt == 0) {
        // 最初のオドメトリーデータで基準姿勢を初期化する
        // オドメトリーデータ（各時刻におけるロボット位置(x,y,th))
        ipose = scan.pose;
        ipose.calRmat();
      }
      mapByOdometry(&scan);
...
void SlamLauncher::mapByOdometry(Scan2D *scan) {
  // スキャンデータをセンサー座標系から地図座標系への変換を行い、地図へ描画する。
  //  Pose2D &pose = scan->pose;               // スキャン取得時のオドメトリ位置
  Pose2D pose;
  // 現在と1つ前のオドメトリーから相対姿勢を算出
  ...
  for (size_t j = 0; j < lps.size(); j++) {
    LPoint2D &lp = lps[j];
    LPoint2D glp;
    // Lidarの点群をグローバル座標（ここではオドメトリーの座標系）に変換
    pose.globalPoint(lp, glp);  // センサ座標系から地図座標系に変換
    glps.emplace_back(glp);
  }

  // 点群地図pcmapにデータを格納
  pcmap->addPose(pose);    // 相対姿勢を格納
  pcmap->addPoints(glps);  //グローバル座標に変換したLidar点群を格納
  pcmap->makeGlobalMap();
